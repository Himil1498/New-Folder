...
import MapControls   from './WorkingMeasurement/Controls/MapControls';
...
if (typeof document !== 'undefined' && !document.querySelector('#elevation-animation-styles')) {
...
const WorkingMeasurementMap = React.forwardRef(({
  ...
  const [pointElevations, setPointElevations] = useState({ point1: null, point2: null });
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          addLog(`✅ Loaded ${mockData.infrastructure_data.length} items from mock database`);
  ...
  const { isInsideIndia, indiaBounds, clearIndiaBoundary, showIndiaBoundary } = useIndiaBoundary(map, {
    strokeColor: '#FF5722',
    strokeOpacity: 0.4,
    strokeWeight: 2,
    fillColor: '#FF5722',
  ...
  const startDrawing = () => {
    ...
    if (streetViewOpen) {
      ...
      addNotification('Drawing mode active! Click on Street View to add points.', 'info');
    } else {
    ...
    clickListenerRef.current = map.addListener('click', (event) => {
      ...
      addLog(`📍 DRAWING CLICK EVENT - Clicked at: ${lat.toFixed(4)}, ${lng.toFixed(4)}`);
      ...
      if (isElevationMode) {
        addLog(`📈 ELEVATION MODE CLICK - Point at: ${lat.toFixed(4)}, ${lng.toFixed(4)}`);
        ...
        if (insideIndia === false) {
          ...
          addNotification('Elevation points must be within India\'s boundary', 'warning');
          return;
        }
        ...
        setElevationPoints(prevPoints => {
          if (prevPoints.length >= 2) {
            ...
            addLog('📈 Elevation points cleared, starting with new first point');
            addNotification('Starting new elevation profile with first point', 'info');
            return newPoints;
          } else {
            ...
            if (pointNumber === 1) {
              ...
              addNotification('First elevation point selected. Click second point.', 'info');
            } else {
              ...
              addNotification(`Two elevation points selected (${formatDistance(distance)} apart). Click "Get Elevation Data" to view profile.`, 'success');
            }
      ...
      addLog(`🔍 Checking India boundary for: ${lat.toFixed(4)}, ${lng.toFixed(4)}`);
      ...
      if (insideIndia === false) {
        ...
        addNotification('Please click within India\'s boundary for measurements', 'warning');
        return;
      } else if (insideIndia === null) {
        addLog('⚠️ India boundary not ready - allowing click');
        addNotification('India boundary data loading... Point added provisionally', 'info');
      ...
      setPoints(prevPoints => {
        ...
        setMarkers(prevMarkers => {
          ...
          addLog(`✅ Added marker ${newPoints.length} to state. Total markers: ${newMarkers.length}`);
          return newMarkers;
        });
        ...
        if (newPoints.length >= 2) {
          ...
          try {
            ...
            // Distance labels removed as requested - measurements shown in sidebar only
  ...
  const startPolygonDrawing = () => {
    ...
    if (streetViewOpen) {
      ...
      addNotification('Polygon drawing active! Click to add polygon points.', 'info');
    } else {
    ...
    clickListenerRef.current = map.addListener('click', (event) => {
      ...
      addLog(`📍 POLYGON CLICK - Clicked at: ${lat.toFixed(4)}, ${lng.toFixed(4)}`);
      ...
      setPolygonPoints(prevPoints => {
        ...
        if (newPoints.length >= 3) {
          ...
          try {
            ...
            // Area labels removed as requested - polygon measurements shown in sidebar only
  ...
  const clearPolygonData = () => {
    ...
    addLog(`📉 Before clear - Points: ${polygonPoints.length}, Area: ${polygonArea}, Perimeter: ${polygonPerimeter}`);
    addNotification('Clearing polygon data...', 'info');

    // Remove path listeners if any
    if (polygonPathListenersRef.current && polygonPathListenersRef.current.length > 0) {
      polygonPathListenersRef.current.forEach(l => {
        try { window.google.maps.event.removeListener(l); } catch (e) {}
      });
    ...
    setTimeout(() => {
      ...
      addLog(`📉 After clear - Points: ${polygonPoints.length}, Area: ${polygonArea}, Perimeter: ${polygonPerimeter}`);
    }, 10);
  ...
  const loadSavedPolygons = () => {
    ...
    setSavedPolygons(polygons.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)));
  };

  const deletePolygon = () => {
    if (polygonToDelete) {
      // Check if the deleted polygon is the one currently loaded on the map
      const isDeletingLoaded = loadedPolygonKey && polygonToDelete.key === loadedPolygonKey;
      ...
      if (isDeletingLoaded) {
        clearPolygonData();
        addNotification(`Deleted and cleared from map: ${polygonToDelete.name}`, 'success');
      } else {
        addNotification(`Deleted polygon: ${polygonToDelete.name}`, 'success');
      }
  ...
  // Load measurement data function for viewing saved distance measurements
  const loadMeasurementData = (measurementData) => {
    try {
      if (!measurementData) return;

      addLog(`📂 Loading saved measurement data: ${measurementData.name}...`);
      
      // Clear existing measurement data first
      clearAll();

      // Set the measurement points and distance
      setPoints(measurementData.points);
      setTotalDistance(measurementData.distance || 0);
      
      // Recreate markers for each point
      const newMarkers = [];
      ...
      // Recreate the polyline if we have at least 2 points
      if (measurementData.points.length >= 2) {
        const newPolyline = new window.google.maps.Polyline({
          path: measurementData.points,
          geodesic: true,
          strokeColor: '#2196F3',
          strokeOpacity: 1.0,
          strokeWeight: 3,
      ...
      // Calculate and display segment distances if available
      if (measurementData.segmentDistances && measurementData.segmentDistances.length > 0) {
        setSegmentDistances(measurementData.segmentDistances);
      } else {
        // Recalculate segment distances if not available
        const segments = [];
        for (let i = 1; i < measurementData.points.length; i++) {
          const distance = calculateDistance(measurementData.points[i-1], measurementData.points[i]);
      ...
      if (measurementData.points.length > 0) {
        const bounds = new window.google.maps.LatLngBounds();
        measurementData.points.forEach(point => bounds.extend(point));
        map.fitBounds(bounds);
      }

      addLog('✅ Measurement data loaded successfully');
      addNotification('Measurement loaded!', 'success');
    } catch (error) {
      addLog(`❌ Failed to load measurement data: ${error.message}`);
      addNotification('Failed to load measurement', 'error');
    }
  };
  ...
  const enableEditLoadedPolygon = () => {
    ...
    const l1 = window.google.maps.event.addListener(path, 'set_at', updateFromPath);
    const l2 = window.google.maps.event.addListener(path, 'insert_at', updateFromPath);
    const l3 = window.google.maps.event.addListener(path, 'remove_at', updateFromPath);
  ...
  const saveEditedPolygon = () => {
    ...
    try {
      ...
      if (polygon && typeof polygon.setEditable === 'function') polygon.setEditable(false);
      if (polygonPathListenersRef.current && polygonPathListenersRef.current.length > 0) {
        polygonPathListenersRef.current.forEach(l => {
          try { window.google.maps.event.removeListener(l); } catch (e) {}
        });
    ...
    try {
      ...
      distanceLabels.forEach((label, index) => {
        try {
          if (label && label.setMap && typeof label.setMap === 'function') {
            ...
            addLog(`⚠️ Distance label ${index + 1} invalid or no setMap method`);
          }
        } catch (error) {
          addLog(`❌ Error removing distance label ${index + 1}: ${error.message}`);
        }
      });
      ...
      if (polygon) {
        try {
          ...
          addLog(`❌ Error removing polygon during clear all: ${error.message}`);
        }
      }
      setPolygon(null);
      ...
      if (areaLabel) {
        try {
          ...
          addLog(`❌ Error removing area label during clear all: ${error.message}`);
        }
      }
      setAreaLabel(null);
      ...
      polygonMarkers.forEach((marker, index) => {
        try {
          ...
          addLog(`❌ Error removing polygon marker ${index + 1}: ${error.message}`);
        }
      });
      setPolygonMarkers([]);
      setPolygonPoints([]);
      ...
      try {
        ...
        if (subPopKmlLayer && subPopKmlLayer.markers) {
          ...
          addLog(`✅ Cleared ${subPopKmlLayer.markers.length} Sub-POP markers`);
        }
      ...
      addLog('✅ All state (including polygons and boundary) cleared successfully');
      addLog('✅ Clear all operation completed!');
      addNotification('All measurements, polygons, and boundary cleared successfully!', 'success');
  ...
  useEffect(() => {
    ...
    script.onerror = () => addLog('❌ Failed to load MarkerClusterer library');
  ...
  const loadKmlLayer = async (url, layerType) => {
    ...
    try {
      ...
      addLog(`✅ ${layerType} KML file fetched, ${kmlText.length} characters`);
      ...
      if (markers.length > 0) {
        ...
        addNotification(`${layerType} locations loaded (${markers.length} items)`, 'success');
        // Add to clusterer if available
        if (clustererLoadedRef.current && window.markerClusterer?.MarkerClusterer) {
          if (layerType === 'POP') {
  ...
  const parseKmlAndCreateMarkers = (kmlText, layerType) => {
    ...
    try {
      ...
      for (let i = 0; i < placemarks.length; i++) {
        ...
        const name = nameElement ? nameElement.textContent : `${layerType} Location ${i + 1}`;
        ...
        if (coordsElement) {
          ...
          if (coords.length >= 2) {
            ...
            if (!isNaN(lat) && !isNaN(lng)) {
              ...
              const infoWindow = new window.google.maps.InfoWindow({
                content: createKMLInfoWindow(name, description, lat, lng, layerType, extended)
              });
              ...
              markers.push(marker);
              addLog(`✅ Created ${layerType} marker: ${name} at ${lat.toFixed(4)}, ${lng.toFixed(4)}`);
            }
          }
        }
      }
  ...
  const createDummyMarkers = (layerType) => {
    ...
    const locations = isPopLayer 
      ? [
          { lat: 28.6139, lng: 77.2090, name: 'Delhi POP' },
          { lat: 19.0760, lng: 72.8777, name: 'Mumbai POP' },
          { lat: 12.9716, lng: 77.5946, name: 'Bangalore POP' }
        ]
      : [
          { lat: 28.5355, lng: 77.3910, name: 'Noida Sub-POP' },
          { lat: 19.1136, lng: 72.8697, name: 'Andheri Sub-POP' },
          { lat: 13.0358, lng: 77.5970, name: 'Whitefield Sub-POP' }
        ];
    ...
    locations.forEach((location, index) => {
      ...
      const infoWindow = new window.google.maps.InfoWindow({
        content: createKMLInfoWindow(location.name, `Test ${layerType} location`, location.lat, location.lng, layerType)
      });
  ...
  const importKMZFile = async (file) => {
    try {
      ...
      const markers = parseKmlAndCreateImportedMarkers(kmlText, 'Imported (KMZ)');
      addImportedMarkers(markers);
    } catch (e) {
  ...
  const extractKMLExtendedData = (placemark) => {
    ...
    // Get ExtendedData
    const extendedDataElements = placemark.getElementsByTagName('ExtendedData')[0];
    ...
    let content = `
      <div style="max-width: 400px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.4;">
        <div style="background: linear-gradient(135deg, #9C27B0, #673AB7); color: white; padding: 15px; margin: -8px -8px 15px -8px; border-radius: 8px 8px 0 0; box-shadow: 0 2px 8px rgba(0,0,0,0.15);">
          <h3 style="margin: 0; font-size: 18px; font-weight: 600;">${data.name || 'Infrastructure Location'}</h3>
    ...
    fields.forEach(field => {
      ...
      if (value && value.toString().trim() && value.toString().trim() !== '') {
        ...
        if (field.key === 'status' || field.label.includes('Status')) {
          const statusColor = displayValue.toLowerCase() === 'active' ? '#4CAF50' : 
                            displayValue.toLowerCase() === 'maintenance' ? '#FF9800' : '#666';
        ...
        if (field.label.includes('Rented')) {
          const rentColor = displayValue.toLowerCase() === 'yes' ? '#4CAF50' : '#F44336';
    ...
    if (!addedFields.has('Latitude')) {
      content += `
        <tr style="border-bottom: 1px solid #f0f0f0;">
          ...
          <td style="padding: 8px 0; color: #333; font-family: monospace;">${lat.toFixed(6)}, ${lng.toFixed(6)}</td>
        </tr>
      `;
    }
          ...
          ">
            <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
              ...
              <tr style="border-bottom: 1px solid #f0f0f0;">
                ...
                <td style="padding: 8px 0; color: #333;">${formatValue(location.id)}</td>
              </tr>
              <tr style="border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 8px 12px 8px 0; font-weight: 600; color: #555;">Network ID:</td>
                <td style="padding: 8px 0; color: #333; word-break: break-word;">${formatValue(location.network_id)}</td>
              </tr>
              <tr style="border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 8px 12px 8px 0; font-weight: 600; color: #555;">Unique ID:</td>
                <td style="padding: 8px 0; color: #333; word-break: break-word;">${formatValue(location.unique_id)}</td>
              </tr>
              <tr style="border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 8px 12px 8px 0; font-weight: 600; color: #555;">Ref Code:</td>
                <td style="padding: 8px 0; color: #333;">${formatValue(location.ref_code)}</td>
              </tr>
              <tr style="border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 8px 12px 8px 0; font-weight: 600; color: #555;">Name:</td>
                <td style="padding: 8px 0; color: #333; word-break: break-word;">${formatValue(location.name)}</td>
              </tr>
              <tr style="border-bottom: 1px solid #f0f0f0;">
              ...
              <tr style="border-bottom: 1px solid #f0f0f0;">
                ...
                <td style="padding: 8px 0; color: #333;">${formatValue(location.created_on, 'created')}</td>
              </tr>
              <tr style="border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 8px 12px 8px 0; font-weight: 600; color: #555;">Updated On:</td>
                <td style="padding: 8px 0; color: #333;">${formatValue(location.updated_on, 'updated')}</td>
              </tr>
              
              <!-- Contact Information -->
              <tr style="border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 8px 12px 8px 0; font-weight: 600; color: #555;">Address:</td>
                <td style="padding: 8px 0; color: #333; word-break: break-word; line-height: 1.3;">${formatValue(location.address)}</td>
              </tr>
              <tr style="border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 8px 12px 8px 0; font-weight: 600; color: #555;">Contact Name:</td>
                <td style="padding: 8px 0; color: #333;">${formatValue(location.contact_name)}</td>
              </tr>
              <tr style="border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 8px 12px 8px 0; font-weight: 600; color: #555;">Contact No:</td>
                <td style="padding: 8px 0; color: #333;">${formatValue(location.contact_no)}</td>
              </tr>
              ...
              <tr style="border-bottom: 1px solid #f0f0f0;">
                ...
                <td style="padding: 8px 0; color: #333;">${formatValue(location.rent_amount, 'rent')}</td>
              </tr>
              <tr style="border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 8px 12px 8px 0; font-weight: 600; color: #555;">Agreement Start:</td>
                <td style="padding: 8px 0; color: #333;">${formatValue(location.agreement_start_date, 'date')}</td>
              </tr>
              <tr style="border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 8px 12px 8px 0; font-weight: 600; color: #555;">Agreement End:</td>
                <td style="padding: 8px 0; color: #333;">${formatValue(location.agreement_end_date, 'date')}</td>
              </tr>
              ` : ''}
              <tr style="border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 8px 12px 8px 0; font-weight: 600; color: #555;">Nature of Business:</td>
                <td style="padding: 8px 0; color: #333; word-break: break-word;">${formatValue(location.nature_of_business)}</td>
              </tr>
              
              <!-- Technical Information -->
              <tr style="border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 8px 12px 8px 0; font-weight: 600; color: #555;">Structure Type:</td>
                <td style="padding: 8px 0; color: #333;">${formatValue(location.structure_type)}</td>
              </tr>
              <tr style="border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 8px 12px 8px 0; font-weight: 600; color: #555;">UPS Availability:</td>
                <td style="padding: 8px 0;">
                  <span style="
                    color: ${location.ups_availability ? '#4CAF50' : '#F44336'}; 
                    font-weight: 600;
              ...
              <tr style="border-bottom: 1px solid #f0f0f0;">
                ...
                <td style="padding: 8px 0; color: #333;">${formatValue(location.backup_capacity)} KVA</td>
              </tr>
              ` : ''}
              ...
              <tr>
                ...
                <td style="padding: 8px 0; color: #333; font-family: monospace;">
                  ${location.position?.lat?.toFixed(6) || 'N/A'}, ${location.position?.lng?.toFixed(6) || 'N/A'}
                </td>
              </tr>
            </table>
          </div>
  ...
  const createKMLInfoWindow = (name, description, lat, lng, layerType, data = null) => {
    ...
    return `
      <div style="max-width: 400px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.4;">
        <div style="background: linear-gradient(135deg, ${headerColor}, ${headerColorDark}); color: white; padding: 15px; margin: -8px -8px 15px -8px; border-radius: 8px 8px 0 0; box-shadow: 0 2px 8px rgba(0,0,0,0.15);">
          <h3 style="margin: 0; font-size: 18px; font-weight: 600;">${name || `${layerType} Location`}</h3>
          <div style="margin-top: 8px; opacity: 0.9; font-size: 13px;">
        ...
        ">
          ...
          ">
            ...
            <tr style="border-bottom: 1px solid #f0f0f0;">
              ...
              <td style="padding: 8px 0; color: #333; word-break: break-word; line-height: 1.3;">${name || 'N/A'}</td>
            </tr>
            <tr style="border-bottom: 1px solid #f0f0f0;">
            ...
            <tr style="border-bottom: 1px solid #f0f0f0;">
              ...
              <td style="padding: 8px 0; color: #333;">${isPopType ? 'Primary Operations Point' : 'Secondary Operations Point'}</td>
            </tr>
            <tr style="border-bottom: 1px solid #f0f0f0;">
              <td style="padding: 8px 12px 8px 0; font-weight: 600; color: #555;">Infrastructure:</td>
              <td style="padding: 8px 0; color: #333;">${isPopType ? 'Main Tower, Full Equipment' : 'Sub Tower, Limited Equipment'}</td>
            </tr>
            <tr style="border-bottom: 1px solid #f0f0f0;">
              <td style="padding: 8px 12px 8px 0; font-weight: 600; color: #555;">Coverage Area:</td>
              <td style="padding: 8px 0; color: #333;">${isPopType ? 'Metropolitan/Regional' : 'Local/Neighborhood'}</td>
            </tr>
            <tr style="border-bottom: 1px solid #f0f0f0;">
              <td style="padding: 8px 12px 8px 0; font-weight: 600; color: #555;">Coordinates:</td>
              <td style="padding: 8px 0; color: #333; font-family: monospace;">${lat.toFixed(6)}, ${lng.toFixed(6)}</td>
    ...
    try {
      ...
      setManualLocations(prev => {
        ...
        if (editingLocation) {
          ...
          const existingLocation = prev[type].find(item => item.id === editingLocation.id);
          if (existingLocation) {
      ...
      localStorage.setItem('manual_infrastructure_locations', JSON.stringify({
      ...
      if (useDatabase) {
        try {
          ...
          if (editingLocation && editingLocation.database_id) {
            ...
            addNotification(`${type.toUpperCase()} location updated in database!`, 'success');
          } else {
            // Create new record
            dbData.created_at = new Date().toISOString();
            const dbResult = await saveToDatabase(dbData);
            addNotification(`${type.toUpperCase()} location saved to database!`, 'success');
            // Save database ID to localStorage for reference
            data.database_id = dbResult.id;
          }
        } catch (error) {
          addLog(`⚠️ Database ${editingLocation ? 'update' : 'save'} failed, saving locally only`);
          addNotification('Saved locally only (database error)', 'warning');
        }
      } else {
        addNotification(`${type.toUpperCase()} location ${actionText.toLowerCase()} locally!`, 'success');
      }
  ...
  const handleManualCoordAdd = async (type) => {
    try {
      ...
      addLog(`🎯 Manual coordinate entry: ${type.toUpperCase()} at ${lat}, ${lng}`);
      ...
      if (enableBoundaryCheck) {
        ...
        const locationCheck = isInsideIndia ? isInsideIndia({ lat, lng }) : null;
        
        if (isInsideIndia && locationCheck !== null) {
          isValidLocation = locationCheck;
          checkMethod = 'polygon';
        } else {
          isValidLocation = (lat >= 6.4 && lat <= 37.6 && lng >= 68.1 && lng <= 97.4);
          checkMethod = 'basic';
        }
        
        if (!isValidLocation) {
          addLog(`❌ Manual coordinates outside India boundary (${checkMethod} check)`);
          addNotification('Coordinates must be within India\'s boundaries', 'error');
          return;
        }
      }
      ...
      addNotification(`Form opened for ${type.toUpperCase()} at manual coordinates`, 'success');
  ...
  const editManualLocation = (location, type) => {
    addLog(`✏️ Starting edit for ${type.toUpperCase()} location: ${location.name}`);
  ...
  const deleteManualLocation = (location, type) => {
    try {
      ...
      if (location.marker) {
        ...
        addLog(`🗺️ Removed marker from map for ${type.toUpperCase()}: ${location.name}`);
      }
      ...
      if (location.infoWindow) {
        ...
        addLog(`💬 Closed info window for ${type.toUpperCase()}: ${location.name}`);
      }
      ...
      addLog(`✅ Successfully deleted ${type.toUpperCase()} location: ${location.name}`);
      addNotification(`🗑️ ${type.toUpperCase()} location "${location.name}" deleted successfully!`, 'success');
  ...
  const refreshClustererAfterDelete = (type, remainingLocations) => {
    ...
    try {
      ...
      if (validMarkers.length > 0) {
        ...
        addLog(`🔗 Refreshed ${type.toUpperCase()} clusterer with ${validMarkers.length} markers`);
      } else {
      ...
      addLog(`❌ Error refreshing ${type.toUpperCase()} clusterer: ${error.message}`);
    }
  };
  ...
  const loadManualLocationsFromDatabase = async () => {
    ...
    try {
      ...
      addLog(`✅ Loaded ${processedData.pop.length} POP and ${processedData.sub.length} Sub-POP locations from database`);
      
    } catch (error) {
      addLog(`⚠️ Failed to load from database, using localStorage: ${error.message}`);
  ...
  const showLocationOnMap = (location, type) => {
    ...
    if (!targetMarker) {
      ...
      addLog(`🔧 Created missing marker for ${type.toUpperCase()} location: ${location.name}`);
    }
    
    // Ensure marker is visible on the map (in case it was hidden due to layer toggle)
    if (targetMarker.getMap() !== map) {
      targetMarker.setMap(map);
      addLog(`👁️ Made marker visible for ${type.toUpperCase()} location: ${location.name}`);
    }
    
    // Create new info window with latest data
    const infoWindow = location.infoWindow || new window.google.maps.InfoWindow({
    ...
    addLog(`📍 Showed ${type.toUpperCase()} location on map: ${location.name}`);
  ...
  const loadManualLocations = () => {
    ...
    ['pop', 'sub'].forEach(type => {
      manualLocations[type].forEach(location => {
        ...
        if ((type === 'pop' && showPopLayer) || (type === 'sub' && showSubPopLayer)) {
          marker.setMap(map);
        }
  ...
  useEffect(() => {
    ...
    if (persistentClickListenerRef.current) {
      window.google.maps.event.removeListener(persistentClickListenerRef.current);
    }
    ...
    persistentClickListenerRef.current = map.addListener('click', (event) => {
      ...
      addLog(`📍 PERSISTENT CLICK (Updated) - Clicked at: ${lat.toFixed(4)}, ${lng.toFixed(4)}`);
      addLog(`📍 Current isAddingLocation state: ${isAddingLocation}`);
      
      // Handle adding location mode (highest priority)
      if (isAddingLocation) {
        addLog(`🔍 Checking boundary for location: ${lat.toFixed(4)}, ${lng.toFixed(4)}`);
        ...
        if (enableBoundaryCheck) {
          ...
          if (isInsideIndia && locationCheck !== null) {
            ...
            isValidLocation = (lat >= 6.4 && lat <= 37.6 && lng >= 68.1 && lng <= 97.4);
            checkMethod = 'basic';
            addLog(`🗺 Using fallback basic boundary check: ${isValidLocation}`);
          }
          
          if (!isValidLocation) {
            addLog(`❌ Location outside India boundary (${checkMethod} check): ${lat.toFixed(4)}, ${lng.toFixed(4)}`);
            ...
            const infoWindow = new window.google.maps.InfoWindow({
              content: `
                <div style="padding: 8px; text-align: center;">
                  ...
                  <div style="font-size: 12px; color: #666;">Location: ${lat.toFixed(6)}, ${lng.toFixed(6)}</div>
            ...
            addNotification(`Location outside India boundary (${lat.toFixed(3)}°, ${lng.toFixed(3)}°)`, 'error');
            setIsAddingLocation(false);
            map.setOptions({ cursor: 'default' });
            return;
          } else {
            addLog(`✅ Location confirmed inside India (${checkMethod} check): ${lat.toFixed(4)}, ${lng.toFixed(4)}`);
          }
        } else {
          addLog(`🚫 Boundary check disabled - allowing location: ${lat.toFixed(4)}, ${lng.toFixed(4)}`);
        }
        
        addLog(`🏗️ Adding ${isAddingLocation.toUpperCase()} location at: ${lat.toFixed(4)}, ${lng.toFixed(4)}`);
        setAddLocationForm({
          open: true,
        ...
        setIsAddingLocation(false);
        map.setOptions({ cursor: 'default' });
        addNotification(`Form opened for ${isAddingLocation.toUpperCase()} location`, 'success');
        return;
      }
      
      // If not in add mode and not drawing mode, just log the click
      if (!isDrawing && !isPolygonDrawing) {
        addLog(`📍 Regular map click at: ${lat.toFixed(4)}, ${lng.toFixed(4)}`);
      }
    });
  ...
  const addImportedMarkers = (markers) => {
    ...
    setImportedMarkers(prev => {
      ...
      if (clustererLoadedRef.current && window.markerClusterer?.MarkerClusterer) {
        if (!importedClusterRef.current) {
          importedClusterRef.current = new window.markerClusterer.MarkerClusterer({ 
            map: showImportedLayer ? map : null, 
            markers: updated 
          });
        } else {
    ...
    addNotification(`Successfully imported ${markers.length} locations`, 'success');
  };
  ...
  const toggleImportedLayer = (show) => {
    try {
      ...
      if (importedMarkers && importedMarkers.length > 0) {
        ...
        addLog(`${show ? 'Showing' : 'Hiding'} ${importedMarkers.length} imported markers`);
      }
      ...
      try {
        ...
        if (MarkerClusterer) {
          if (!importedClusterRef.current && importedMarkers.length > 0) {
            ...
            addLog(`✅ Imported clusterer created and ${show ? 'shown' : 'hidden'}`);
      ...
      addNotification(`Error toggling imported layer: ${error.message}`, 'error');
    }
  };
...
Whitefield Sub-POP,13.0358,77.5970,Sub-POP,Active,"Whitefield, Bangalore, Karnataka, India",Meera Iyer,9777888999,REF006,Secondary Operations,Rooftop,Yes,18000,2024-03-15,2025-03-14,No,0`;
  ...
  const exportManualDataCSV = () => {
    try {
      const allLocations = [
        ...manualLocations.pop.map(loc => ({ ...safeSerialize(loc), type: 'POP' })),
        ...manualLocations.sub.map(loc => ({ ...safeSerialize(loc), type: 'Sub-POP' }))
      ];
      ...
      // Create CSV header
      const headers = ['Name', 'Latitude', 'Longitude', 'Type', 'Status', 'Address', 'Contact Name', 'Contact Number', 'Ref Code', 'Nature of Business', 'Structure Type', 'Is Rented', 'Rent Amount', 'Agreement Start Date', 'Agreement End Date', 'UPS Availability', 'Backup Capacity', 'Created On', 'Updated On'];
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `manual_infrastructure_export_${new Date().toISOString().split('T')[0]}.csv`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
      ...
      addNotification(`Successfully exported ${allLocations.length} locations to CSV`, 'success');
  ...
  const exportManualDataXLSX = async () => {
    try {
      ...
      // Generate and download the file
      const fileName = `manual_infrastructure_export_${new Date().toISOString().split('T')[0]}.xlsx`;
      window.XLSX.writeFile(wb, fileName);
      
      addLog(`✅ Exported ${allLocations.length} manual locations to XLSX`);
      addNotification(`Successfully exported ${allLocations.length} locations to XLSX`, 'success');
...
`;
      ...
      allLocations.forEach((location, index) => {
        ...
        const name = location.name || `${location.type} Location ${index + 1}`;
        const description = `
          <![CDATA[
            <b>Type:</b> ${location.type}<br/>
            <b>Status:</b> ${location.status || 'N/A'}<br/>
            <b>Address:</b> ${location.address || 'N/A'}<br/>
            <b>Contact:</b> ${location.contact_name || 'N/A'} (${location.contact_no || 'N/A'})<br/>
            <b>Structure:</b> ${location.structure_type || 'N/A'}<br/>
            <b>UPS:</b> ${location.ups_availability ? 'Available' : 'Not Available'}<br/>
      ...
      <styleUrl>#${location.type === 'POP' ? 'popStyle' : 'subPopStyle'}</styleUrl>
      <Point>
...
`;
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `manual_infrastructure_export_${new Date().toISOString().split('T')[0]}.kml`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
      ...
      addNotification(`Successfully exported ${allLocations.length} locations to KML`, 'success');
...
`;
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `manual_infrastructure_export_${new Date().toISOString().split('T')[0]}.kmz`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
      ...
      addNotification(`Successfully exported ${allLocations.length} locations to KMZ`, 'success');
  ...
  const fetchElevationData = async (pathPoints) => {
    if (!pathPoints || pathPoints.length < 2) {
      addNotification('Need at least 2 points to get elevation data', 'warning');
      return;
    }
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        elevationServiceRef.current.getElevationAlongPath({
          ...
          if (status === 'OK' && results) {
            addLog(`✅ Retrieved elevation data for ${results.length} points`);
            resolve(results);
          } else {
      ...
      addNotification(`Error fetching elevation data: ${error.message}`, 'error');
      throw error;
    }
  };
    ...
    elevationResults.forEach((result, index) => {
      ...
      if (index > 0) {
        ...
        const segmentDistance = window.google.maps.geometry.spherical.computeDistanceBetween(
          new window.google.maps.LatLng(prevResult.location.lat(), prevResult.location.lng()),
          new window.google.maps.LatLng(result.location.lat(), result.location.lng())
        );
  ...
  const createElevationChart = (elevationData) => {
    ...
    addLog('✅ Elevation chart created with highlighted highest and lowest points');
  };
  ...
  const createElevationMarkers = (elevationData, stats) => {
    ...
    const maxPoint = elevationData.find(d => d.elevation === stats.maxElevation);
    const minPoint = elevationData.find(d => d.elevation === stats.minElevation);
      ...
      const highInfoWindow = new window.google.maps.InfoWindow({
        content: `
          <div style="padding: 8px; font-family: Arial, sans-serif;">
            ...
            <p style="margin: 4px 0;"><strong>Elevation:</strong> ${maxPoint.formattedElevation}</p>
            <p style="margin: 4px 0;"><strong>Distance:</strong> ${maxPoint.formattedDistance}</p>
            <p style="margin: 4px 0;"><strong>Coordinates:</strong> ${maxPoint.lat.toFixed(6)}, ${maxPoint.lng.toFixed(6)}</p>
          </div>
        `
      });
      ...
      const lowInfoWindow = new window.google.maps.InfoWindow({
        content: `
          <div style="padding: 8px; font-family: Arial, sans-serif;">
            ...
            <p style="margin: 4px 0;"><strong>Elevation:</strong> ${minPoint.formattedElevation}</p>
            <p style="margin: 4px 0;"><strong>Distance:</strong> ${minPoint.formattedDistance}</p>
            <p style="margin: 4px 0;"><strong>Coordinates:</strong> ${minPoint.lat.toFixed(6)}, ${minPoint.lng.toFixed(6)}</p>
          </div>
        `
      });
  ...
  const getElevationForPath = async () => {
    ...
    try {
      ...
      if (processedData.length > 0) {
        ...
        addNotification(`Elevation profile updated with ${processedData.length} points`, 'success');
      }
  ...
  const addElevationPointMarker = (position, pointNumber) => {
    ...
    const infoWindow = new window.google.maps.InfoWindow({
      content: `
        <div style="padding: 8px; font-family: Arial, sans-serif;">
          <h4 style="margin: 0 0 8px 0; color: ${pointNumber === 1 ? '#4CAF50' : '#FF5722'};">📍 Elevation Point ${pointNumber}</h4>
    ...
    addLog(`📍 Added elevation point ${pointNumber} marker at ${position.lat.toFixed(4)}, ${position.lng.toFixed(4)}`);
  };
  ...
  const createElevationPolyline = (points) => {
    ...
    addLog(`✅ Created elevation polyline connecting ${points.length} points`);
  };
  
  // Get elevation data for two selected points
  const getElevationForTwoPoints = async () => {
    if (elevationPoints.length !== 2) {
      addNotification('Please select exactly 2 points on the map first', 'warning');
      return;
    }
    ...
    try {
      ...
      const processedData = processElevationData(elevationResults, elevationPoints);
      const stats = calculateElevationStats(processedData);
  ...
  const togglePopLayer = async (show) => {
    try {
      ...
      if (popKmlLayer && popKmlLayer.markers) {
        ...
        addLog(`${show ? 'Showing' : 'Hiding'} ${popKmlLayer.markers.length} POP KML markers`);
      }
      ...
      if (manualLocations && manualLocations.pop) {
        ...
        addLog(`${show ? 'Showing' : 'Hiding'} ${manualLocations.pop.length} manual POP markers`);
      }
  ...
  const toggleSubPopLayer = async (show) => {
    try {
      if (show && !subPopKmlLayer) {
        ...
        const layerData = await loadKmlLayer('/sub_pop_location.kml', 'Sub-POP');
      ...
      if (subPopKmlLayer && subPopKmlLayer.markers) {
        ...
        addLog(`${show ? 'Showing' : 'Hiding'} ${subPopKmlLayer.markers.length} Sub-POP KML markers`);
      }
      ...
      if (manualLocations && manualLocations.sub) {
        ...
        addLog(`${show ? 'Showing' : 'Hiding'} ${manualLocations.sub.length} manual Sub-POP markers`);
      }
      ...
      addNotification(`Error toggling Sub-POP layer: ${error.message}`, 'error');
    }
  };
    ...
    try {
      ...
      if (streetViewOpen) {
        ...
        if (points.length > 0) {
          ...
          addLog(`🎯 Using first measurement point: ${targetPosition.lat.toFixed(4)}, ${targetPosition.lng.toFixed(4)}`);
        } else {
          const center = map.getCenter();
          targetPosition = { lat: center.lat(), lng: center.lng() };
          addLog(`🎯 Using map center: ${targetPosition.lat.toFixed(4)}, ${targetPosition.lng.toFixed(4)}`);
        }
        ...
        streetView.setPosition(new window.google.maps.LatLng(targetPosition.lat, targetPosition.lng));
  ...
  const refreshMeasurementLabelsForStreetView = () => {
    ...
    // Refresh current measurement markers to ensure visibility in Street View
    markers.forEach((marker, index) => {
      if (marker && marker.setMap) {
        // Ensure markers are visible in Street View
        marker.setMap(map);
        
        // Update marker visibility and icons for Street View
        marker.setOptions({
          icon: {
            path: window.google.maps.SymbolPath.CIRCLE,
            scale: 12, // Larger for Street View visibility
            fillColor: '#FF5722',
            fillOpacity: 1,
            strokeColor: '#FFFFFF',
            strokeWeight: 3,
          },
          zIndex: 2000, // Higher z-index for Street View
        });
        
        addLog(`✅ Measurement marker ${index + 1} refreshed for Street View`);
      }
    });
    
    // Refresh polygon markers to ensure visibility in Street View
    polygonMarkers.forEach((marker, index) => {
      if (marker && marker.setMap) {
        marker.setMap(map);
        marker.setOptions({
        ...
        addLog(`✅ Polygon marker ${index + 1} refreshed for Street View`);
      }
    });
    
    // Refresh elevation point markers if any
    elevationPointMarkers.forEach((marker, index) => {
      if (marker && marker.setMap) {
        marker.setMap(map);
        marker.setOptions({
          icon: {
        ...
        addLog(`✅ Elevation marker ${index + 1} refreshed for Street View`);
      }
    });
    
    // Refresh polylines to ensure visibility in Street View
    if (polyline) {
      polyline.setOptions({
        strokeWeight: 4, // Thicker line for Street View
        strokeOpacity: 0.9,
        zIndex: 1500,
      });
      addLog('✅ Measurement polyline refreshed for Street View');
    }
    
    // Refresh polygon to ensure visibility in Street View  
    if (polygon) {
      polygon.setOptions({
        strokeWeight: 4,
        strokeOpacity: 0.8,
      ...
      addLog('✅ Polygon refreshed for Street View');
    }
    
    addLog('✅ Street View measurement refresh completed - markers and lines enhanced for visibility');
  };
  
  // Handle elevation click events
  const handleElevationClick = (event) => {
    ...
    addLog(`📊 Elevation click at: ${lat.toFixed(6)}, ${lng.toFixed(6)}`);
    ...
    if (elevationPoints.length < 2) {
      ...
      // Create marker for elevation point
      const marker = new window.google.maps.Marker({
        position: { lat, lng },
        map: map,
        title: `Elevation Point ${newPoints.length}`,
        icon: {
          path: window.google.maps.SymbolPath.CIRCLE,
          scale: 10,
          fillColor: '#FF9800',
      ...
      const newMarkers = [...elevationPointMarkers, marker];
      setElevationPointMarkers(newMarkers);
      
      addLog(`📍 Added elevation point ${newPoints.length}: ${lat.toFixed(6)}, ${lng.toFixed(6)}`);
  ...
  const generateElevationProfile = async (points) => {
    try {
      addLog('📊 Generating elevation profile...');
      
      // Create polyline between the two points
      const profileLine = new window.google.maps.Polyline({
        path: points,
        geodesic: true,
        strokeColor: '#FF9800',
        strokeOpacity: 1.0,
      ...
      setElevationPolyline(profileLine);
      
      // Calculate distance
      const distance = calculateDistance(points[0], points[1]);
      setElevationDistance(distance);
      
      // Mock elevation data generation (in real app, use Google Elevation API)
      const elevationSteps = [];
      const steps = 50;
      
      for (let i = 0; i <= steps; i++) {
        const ratio = i / steps;
        const lat = points[0].lat + (points[1].lat - points[0].lat) * ratio;
        const lng = points[0].lng + (points[1].lng - points[0].lng) * ratio;
        
        // Mock elevation calculation (replace with real API call)
        const elevation = Math.random() * 1000 + 100 + Math.sin(ratio * Math.PI * 4) * 200;
        const stepDistance = ratio * distance;
      ...
      setElevationData(elevationSteps);
      setShowElevationProfile(true);
      
      // Calculate elevation statistics
      const elevations = elevationSteps.map(step => step.elevation);
      const maxElevation = Math.max(...elevations);
      const minElevation = Math.min(...elevations);
      const avgElevation = Math.round(elevations.reduce((sum, elev) => sum + elev, 0) / elevations.length);
      
      setElevationStats({
        maxElevation,
        minElevation,
        totalElevationGain: maxElevation - minElevation,
        totalElevationLoss: 0, // Would be calculated from actual profile
        avgElevation
      });
      
      addLog(`📊 Generated elevation profile: Max: ${maxElevation}m, Min: ${minElevation}m, Avg: ${avgElevation}m`);
      addNotification('Elevation profile generated successfully!', 'success');
      
      // Notify parent component if callback exists
      if (typeof onElevationGenerated === 'function') {
        onElevationGenerated(elevationSteps, points);
      }
      
    } catch (error) {
      addLog(`❌ Error generating elevation profile: ${error.message}`);
      addNotification('Failed to generate elevation profile', 'error');
    }
  };
  ...
  const refreshMeasurements = () => {
    ...
    const hasDistanceData = markers.length > 0 || distanceLabels.length > 0 || polyline;
    ...
    try {
      addLog(`🔄 Refreshing ${markers.length} markers and ${distanceLabels.length} labels`);
      ...
      if (areaLabel && areaLabel.setMap && typeof areaLabel.setMap === 'function') {
  ...
  React.useImperativeHandle(ref, () => ({
    ...
    refreshMeasurementLabelsForStreetView,
    // Save functions (removed duplicate saving - Dashboard handles this now)
    saveMeasurement: (name) => {
      // Just log - Dashboard handles the actual saving
      addLog(`💾 Dashboard requested save: ${name}`);
    },
    
    savePolygonData: (name) => {
      // Just log - Dashboard handles the actual saving
      addLog(`💾 Dashboard requested polygon save: ${name}`);
    },
    ...
    // Elevation controls
    enableElevationMode: (enabled) => {
      addLog(`📊 Dashboard calling enableElevationMode(${enabled})`);
      setIsElevationMode(enabled);
      
      if (enabled) {
        // Clear any previous elevation data
        setElevationPoints([]);
        setElevationPointMarkers([]);
        setElevationData([]);
        setShowElevationProfile(false);
        
        // Add map click listener for elevation points
        if (map && !clickListenerRef.current) {
          clickListenerRef.current = map.addListener('click', handleElevationClick);
          addLog('📊 Added elevation click listener to map');
        }
      } else {
        // Remove click listener and clear elevation data
        if (clickListenerRef.current) {
          window.google.maps.event.removeListener(clickListenerRef.current);
          clickListenerRef.current = null;
          addLog('📊 Removed elevation click listener from map');
        }
        
        // Clear elevation markers and data
        elevationPointMarkers.forEach(marker => {
  ...
  }), [map, points, polygonPoints, totalDistance, polygonArea, segmentDistances, polygonPerimeter, loadMeasurementData, loadPolygonData]);
  ...
  const deleteMeasurement = () => {
    if (measurementToDelete) {
      ...
      loadSavedMeasurements();
      addNotification(`Deleted measurement: ${measurementToDelete.name}`, 'success');
      ...
      if (map && measurement.points.length > 0) {
        ...
        if (measurement.points.length >= 2) {
          try {
            ...
            addLog(`📌 Creating ${measurement.points.length - 1} distance labels...`);
            for (let i = 1; i < measurement.points.length; i++) {
              try {
                ...
                addLog(`❌ Error creating distance label ${i}: ${error.message}`);
              }
            }
            
            // Update distance labels state
            setDistanceLabels(newDistanceLabels);
            addLog(`✅ Set ${newDistanceLabels.length} distance labels in state`);
    ...
    <Box sx={{ 
          ...
          <Button 
            ...
            onClick={() => {
              ...
              if (!showElevation) {
                ...
                addLog('📈 Elevation mode activated - Click 2 points on map');
                addNotification('Elevation mode: Click 2 points on the map to create elevation profile', 'info');
              } else {
        ...
        <Stack direction="row" spacing={1} alignItems="center" sx={{ mt: 1 }}>
          ...
          <Box sx={{ mr: 1 }}>
            <MapControls
              ...
              onZoomIn={zoomIn}
              onZoomOut={zoomOut}
              onCenterOnIndia={centerOnIndia}
            />
          </Box>
          
          <Tooltip title={streetViewOpen ? "Close Street View" : "Open Street View"}>
            <IconButton 
      ...
      <Box sx={{ 
        ...
        <Box sx={{ flex: 1, position: 'relative' }}>
          <Paper elevation={2} sx={{ height: '100%', position: 'relative' }}>
            ...
            {!isDrawing && points.length === 0 && loaded && showInstructions && (
            ...
            <Box sx={{ position: 'absolute', bottom: 16, left: 16, zIndex: 1000 }}>
              <Chip 
                label={`Boundary loaded: ${indiaBounds ? 'Yes' : 'Loading...'}`}
                ...
                <Stack spacing={2}>
                  ...
                  {elevationPoints.length === 2 && (
                    <Box sx={{ mt: 1, pt: 1, borderTop: '1px solid #E0E0E0' }}>
                      ...
                      {pointElevations.point1 && pointElevations.point2 && (
                        <Box>
                          <Typography variant="body2" sx={{ 
                            color: pointElevations.point1.elevation > pointElevations.point2.elevation ? '#2E7D32' : '#C62828',
                            fontWeight: 'bold',
                            mb: 0.5
                          }}>
                            📍 Higher Point: Point {pointElevations.point1.elevation > pointElevations.point2.elevation ? '1' : '2'} ({Math.max(pointElevations.point1.elevation, pointElevations.point2.elevation).toFixed(0)}m)
                          </Typography>
                          <Typography variant="body2" sx={{ 
                            color: pointElevations.point1.elevation < pointElevations.point2.elevation ? '#2E7D32' : '#C62828',
                            fontWeight: 'bold',
                            ...
                            📍 Lower Point: Point {pointElevations.point1.elevation < pointElevations.point2.elevation ? '1' : '2'} ({Math.min(pointElevations.point1.elevation, pointElevations.point2.elevation).toFixed(0)}m)
                          </Typography>
                          <Typography variant="body2" sx={{ 
                            color: '#FF9800', 
                            fontWeight: 'bold',
                            ...
                            ⚡ Elevation Difference: {Math.abs(pointElevations.point2.elevation - pointElevations.point1.elevation).toFixed(0)}m
                          </Typography>
                          {Math.abs(pointElevations.point2.elevation - pointElevations.point1.elevation) > 0 && (
                            <Typography variant="caption" sx={{ 
                              ...
                              Gradient: {((Math.abs(pointElevations.point2.elevation - pointElevations.point1.elevation) / elevationDistance) * 100).toFixed(2)}%
                            </Typography>
                          )}
                        </Box>
                      )}
        ...
        {showInfrastructure && (
          ...
          >
            ...
            {/* KML Layer Controls */}
            <Paper elevation={1} sx={{ p: 2, mb: 3, bgcolor: '#f8f9fa', borderRadius: 2 }}>
              ...
              <Stack spacing={1}>
                <FormControlLabel
                  ...
                  label={
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <CellTower sx={{ mr: 1, color: '#FF6B35', fontSize: 20 }} />
                ...
                <FormControlLabel
                  ...
                  label={
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <Satellite sx={{ mr: 1, color: '#4CAF50', fontSize: 20 }} />
                      <Typography variant="body2" sx={{ fontWeight: 500 }}>
                        Sub-POP Locations
                      </Typography>
                    </Box>
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      ...
                      onChange={(e) => toggleImportedLayer(e.target.checked)}
                      icon={<CheckBoxOutlineBlank />}
                      checkedIcon={<CheckBox />}
                      sx={{ color: '#9C27B0' }}
                    />
                  ...
                  label={
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <CloudDownload sx={{ mr: 1, color: '#9C27B0', fontSize: 20 }} />
                      <Typography variant="body2" sx={{ fontWeight: 500 }}>
            ...
            <Paper elevation={1} sx={{ p: 2, mb: 2, bgcolor: '#f9f9f9', borderRadius: 2 }}>
              <Typography variant="subtitle2" gutterBottom sx={{ color: '#666', fontWeight: 'bold' }}>
                📊 Data Summary
              </Typography>
              ...
              <Typography variant="caption" sx={{ color: '#999', display: 'block', textAlign: 'center', mt: 1 }}>
                Source: {useDatabase ? 'Database + LocalStorage' : 'LocalStorage Only'}
              </Typography>
            </Paper>
            ...
            <Stack direction="row" spacing={1} sx={{ mb: 2 }}>
              <Button variant="outlined" onClick={downloadCsvTemplate} sx={{ textTransform: 'none', borderRadius: 2 }}>
                Download CSV Template
              </Button>
              <Button variant="outlined" onClick={downloadXlsxTemplate} sx={{ textTransform: 'none', borderRadius: 2 }}>
                Download XLSX Template
              </Button>
            </Stack>
            
            {/* Export Data Section */}
            <Typography variant="subtitle2" gutterBottom sx={{ color: '#666', fontWeight: 'bold', mt: 2 }}>
              Export Manual Data
            </Typography>
            ...
            <Stack spacing={1} sx={{ mb: 2 }}>
              <Stack direction="row" spacing={1}>
                <Button 
                  ...
                  startIcon={exportingData === 'CSV' ? <Settings className="spin" /> : <CloudDownload />}
                  sx={{ 
                    textTransform: 'none', 
                    borderRadius: 2,
                    borderColor: '#4CAF50',
                ...
                <Button 
                  ...
                  disabled={exportingData !== null}
                  startIcon={exportingData === 'XLSX' ? <Settings className="spin" /> : <CloudDownload />}
                  sx={{ 
                    textTransform: 'none', 
                    borderRadius: 2,
                    borderColor: '#4CAF50',
              ...
              <Stack direction="row" spacing={1}>
                <Button 
                  ...
                  disabled={exportingData !== null}
                  startIcon={exportingData === 'KML' ? <Settings className="spin" /> : <CloudDownload />}
                  sx={{ 
                    textTransform: 'none', 
                    borderRadius: 2,
                    borderColor: '#2196F3',
                ...
                <Button 
                  ...
                  disabled={exportingData !== null}
                  startIcon={exportingData === 'KMZ' ? <Settings className="spin" /> : <CloudDownload />}
                  sx={{ 
                    textTransform: 'none', 
                    borderRadius: 2,
                    borderColor: '#2196F3',
            ...
            {/* Data Source Toggle */}
            <Paper elevation={1} sx={{ p: 2, mb: 2, bgcolor: useDatabase ? '#e8f5e8' : '#f8f9fa', borderRadius: 2 }}>
              <FormControlLabel
                control={
                  <Switch
                    checked={useDatabase}
                    onChange={(e) => {
                      ...
                      localStorage.setItem('use_database_storage', JSON.stringify(enabled));
                      addLog(`💾 Data storage switched to ${enabled ? 'database' : 'localStorage'}`);
                      addNotification(`Now using ${enabled ? 'database' : 'local storage'} for data`, 'info');
                    }}
                    color="success"
                  />
                }
            ...
            <Paper elevation={1} sx={{ p: 2, mb: 2, bgcolor: enableBoundaryCheck ? '#f8f9fa' : '#fff3e0', borderRadius: 2 }}>
              <FormControlLabel
                control={
                  <Switch
                    ...
                    onChange={(e) => {
                      ...
                      localStorage.setItem('enable_boundary_check', JSON.stringify(enabled));
                      addLog(`📍 Boundary checking ${enabled ? 'enabled' : 'disabled'}`);
                    }}
                    color="primary"
                  />
                }
                label={
                  <Box>
                    ...
                    <Typography 
                      variant="caption" 
                      color={enableBoundaryCheck ? 'text.secondary' : 'warning.main'}
                      sx={{ fontWeight: enableBoundaryCheck ? 'normal' : 'bold' }}
                    >
                      {enableBoundaryCheck 
                        ? `Boundary restriction active ${indiaBounds ? '(Polygon data loaded)' : '(Basic range check)'}` 
            ...
            <Paper elevation={1} sx={{ p: 2, mb: 2, bgcolor: '#f0f8ff', borderRadius: 2 }}>
              <FormControlLabel
                control={
                  <Switch
              ...
              {showManualEntry && (
                <Box sx={{ mt: 2 }}>
                  {!enableBoundaryCheck && (
                    <Alert severity="warning" sx={{ mb: 2, fontSize: '0.75rem' }} icon={<LocationOn />}>
                      Boundary restriction is disabled. Coordinates outside India will be accepted.
                    </Alert>
                  )}
                  <Grid container spacing={2}>
                    <Grid item xs={6}>
            ...
            <Paper elevation={1} sx={{ p: 2, mb: 2, bgcolor: '#fff8e1', borderRadius: 2 }}>
              <FormControlLabel
                control={
                  <Switch
                    checked={showManageLocations}
                    onChange={(e) => setShowManageLocations(e.target.checked)}
                    color="warning"
                  />
                }
                label={
              ...
              {showManageLocations && (
                <Box sx={{ mt: 2, maxHeight: 300, overflow: 'auto' }}>
                  ...
                  {manualLocations.pop.length > 0 && (
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#FF6B35', fontWeight: 'bold', mb: 1 }}>
                        POP Locations ({manualLocations.pop.length})
                      </Typography>
                      {manualLocations.pop.map((location, index) => (
                        <Card key={location.id || index} variant="outlined" sx={{ mb: 1, p: 1 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <Box 
                              ...
                              onClick={() => showLocationOnMap(location, 'pop')}
                            >
                              <Typography variant="body2" fontWeight="bold" sx={{ color: '#FF6B35' }}>
                                📍 {location.name || `POP Location ${index + 1}`}
                              </Typography>
                              <Typography variant="caption" color="text.secondary">
                                {location.position?.lat?.toFixed(4)}, {location.position?.lng?.toFixed(4)} • Click to view
                              </Typography>
                            </Box>
                            <Box>
                              <IconButton 
                                size="small" 
                                onClick={() => editManualLocation(location, 'pop')}
                                sx={{ color: '#1976D2' }}
                              >
                              ...
                              <IconButton 
                                size="small" 
                                onClick={() => setDeleteConfirmLocation({ ...location, type: 'pop' })}
                                sx={{ color: '#d32f2f' }}
                              >
                  ...
                  {manualLocations.sub.length > 0 && (
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#4CAF50', fontWeight: 'bold', mb: 1, mt: 2 }}>
                        ...
                        <Card key={location.id || index} variant="outlined" sx={{ mb: 1, p: 1 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <Box 
                              ...
                              onClick={() => showLocationOnMap(location, 'sub')}
                            >
                              <Typography variant="body2" fontWeight="bold" sx={{ color: '#4CAF50' }}>
                                📍 {location.name || `Sub-POP Location ${index + 1}`}
                              </Typography>
                              <Typography variant="caption" color="text.secondary">
                                {location.position?.lat?.toFixed(4)}, {location.position?.lng?.toFixed(4)} • Click to view
                              </Typography>
                            ...
                            <Box>
                              <IconButton 
                                size="small" 
                                onClick={() => editManualLocation(location, 'sub')}
                                sx={{ color: '#1976D2' }}
                              >
                                <Tooltip title="Edit Location">
                                  <Settings fontSize="small" />
                              ...
                              <IconButton 
                                size="small" 
                                onClick={() => setDeleteConfirmLocation({ ...location, type: 'sub' })}
                                sx={{ color: '#d32f2f' }}
                              >
                                <Tooltip title="Delete Location">
                                  <Delete fontSize="small" />
                  ...
                  {manualLocations.pop.length === 0 && manualLocations.sub.length === 0 && (
              ...
              <Button
                ...
                onClick={() => {
                  if (isAddingLocation === 'pop') {
                    ...
                    addNotification('🎯 Click within India\'s boundaries to add POP location. Red area shows valid region.', 'info');
                  }
                }}
                sx={{ 
                  fontWeight: 'bold',
              ...
              <Button
                ...
                onClick={() => {
                  if (isAddingLocation === 'sub') {
                    ...
                    addNotification('🎯 Click within India\'s boundaries to add Sub-POP location. Red area shows valid region.', 'info');
                  }
                }}
                sx={{ 
                  fontWeight: 'bold',
      ...
      >
        ...
        <DialogContent>
          ...
          <Typography variant="body2" color="text.secondary">
            Distance: {measurementToDelete?.formattedDistance} • 
      ...
      {/* Polygon Save Dialog */}
      <Dialog open={polygonSaveDialogOpen} onClose={() => setPolygonSaveDialogOpen(false)}>
      ...
      >
                    ...
                    <ListItemSecondaryAction>
                      <Stack direction="row" spacing={1}>
                        ...
                        <Button
                          size="small"
                          variant="outlined"
                          onClick={() => { loadPolygonData(polygon); setPolygonHistoryDialogOpen(false); }}
                        >
                          Load
                        </Button>
                        <IconButton
        ...
        <DialogActions>
          <Button onClick={() => setPolygonHistoryDialogOpen(false)}>Close</Button>
        </DialogActions>
      </Dialog>
      ...
      >
        ...
        <DialogContent>
          ...
          <Typography variant="body2" color="text.secondary">
            ...
            Perimeter: {polygonToDelete ? formatDistance(polygonToDelete.perimeter) : ''} • 
            Points: {polygonToDelete?.points?.length || 0} • 
            Created: {polygonToDelete?.date}
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        <DialogTitle>
          <Stack direction="row" alignItems="center" spacing={2}>
            <Delete color="error" />
            <Typography variant="h6">Delete {deleteConfirmLocation?.type?.toUpperCase()} Location</Typography>
          </Stack>
        </DialogTitle>
        <DialogContent>
          <Alert severity="warning" sx={{ mb: 2 }}>
          ...
          <Typography variant="body1">
            Are you sure you want to delete this {deleteConfirmLocation?.type?.toUpperCase()} location:
          </Typography>
          <Typography variant="h6" color="primary.main" sx={{ mt: 1, mb: 1 }}>
            "{deleteConfirmLocation?.name || 'Unnamed Location'}"
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Coordinates: {deleteConfirmLocation?.position?.lat?.toFixed(4)}, {deleteConfirmLocation?.position?.lng?.toFixed(4)}<br />
            Address: {deleteConfirmLocation?.address || 'No address specified'}<br />
            {deleteConfirmLocation?.database_id && `Database ID: ${deleteConfirmLocation.database_id}`}
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setDeleteConfirmLocation(null)} variant="outlined">
            Cancel
          </Button>
          <Button 
            onClick={() => {
              const locationName = deleteConfirmLocation.name || 'Unnamed Location';
              const locationType = deleteConfirmLocation.type?.toUpperCase();
              addLog(`🗑️ User confirmed deletion of ${locationType}: ${locationName}`);
              deleteManualLocation(deleteConfirmLocation, deleteConfirmLocation.type);
              setDeleteConfirmLocation(null);
              // Give user feedback about the deletion process
              addNotification(`🔄 Processing deletion of ${locationType} location...`, 'info');
            }} 
            variant="contained" 
            color="error"
            startIcon={<Delete />}

...
export default function useIndiaBoundary(map, options = {}) {
  ...
  useEffect(() => {
    ...
    if (!window.google.maps.geometry) {
      console.error(
        "Google Maps geometry library not loaded! Make sure to include it in the script."
      );
      return;
    }
    ...
    fetch("/india.json")
      ...
      .then((data) => {
        ...
        data.features.forEach((feature) => {
          ...
          console.log(
            `Processing feature: ${feature.properties?.st_nm}, Type: ${geometry.type}`
          );
        ...
        console.log(
          `Loaded ${createdPolygons.length} polygons for India boundary`
        );
  ...
  const isInsideIndia = (latLng) => {
    ...
    try {
      ...
      console.log(
        `Point ${latLng.lat.toFixed(4)}, ${latLng.lng.toFixed(
          4
        )} inside India: ${result}`
      );
      return result;
